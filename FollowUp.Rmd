...
full_sce <- make_or_load_full_sce(
    ...
    #  file at this location, but in later passes this will load the saved
    #  version!
...
file used above, but with its ID column mapping to to the cell 'id' metadata of
...
- `pseudocount`: Number, 0.000001 by default, a small value relative to the lowest expected cell frequencies of the data, which is added to both `group.1` and `group.2` median frequencies in fold_change calculations simply to prevent division by zero in fold_change calculation. This pseudocount has no effect on p-values.
